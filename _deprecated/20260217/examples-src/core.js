...
export class CircuitBreakerError extends Error {
    consecutiveFailures;
    constructor(consecutiveFailures) {
...
export class FractalProcessor {
     ...
     */
    async withTimeout(fn, timeoutMs, phase) {
     ...
     */
    async processWithMetadata(text, options, callbacks) {
            ...
            const processCore = async () => {
                ...
                if (this.config.parallelProcessing) {
                    ...
                    const chunkPromises = chunks.map(async (chunk, i) => {
                        ...
                        return this.processChunkWithHandling(chunk, i, chunks.length, globalContext, undefined, // No context propagation in parallel mode
                        options, startTime);
                    });
                    ...
                    for (let i = 0; i < chunks.length; i++) {
                        ...
                        const result = await this.processChunkWithHandling(chunks[i], i, chunks.length, globalContext, previousSummary, options, startTime);
                        if (result) {
     ...
     */
    async processChunkWithHandling(chunk, index, total, globalContext, previousSummary, options, startTime) {
        ...
        try {
            const result = await this.withRetry(() => options.processChunk(chunk, context), index);
     ...
     */
    async *processStream(text, options) {
        const chunks = this.splitIntoChunks(text);
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            try {
                const result = await this.withRetry(() => options.processChunk(chunks[i], context), i);
 ...
 */
export function simpleMerge(results, minCount) {
 ...
 */
export function weightedMerge(results, getKey, minCount) {
